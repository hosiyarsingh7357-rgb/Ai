...
export function TemporalAnalysisChart({ data, type = 'pnl' }: TemporalAnalysisChartProps) {
  ...
  return (
    <ResponsiveContainer width="100%" height="100%">
      <BarChart data={data}>
        ...
        />
        <Bar dataKey="pnl" radius={[4, 4, 0, 0]} fill={type === 'session' ? '#8B5CF6' : undefined}>
          {type === 'pnl' && data.map((entry: any, index: number) => (
