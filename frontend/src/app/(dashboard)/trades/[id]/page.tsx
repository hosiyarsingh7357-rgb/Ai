...
export default function TradeDetailPage() {
  ...
  // aiAnalysis is stored as Json in Prisma, cast it
              ...
              <div className="space-y-4">
                {trade.legs?.map((leg: any) => (
                  <div key={leg.id} className="flex items-center justify-between p-4 bg-[#1E293B]/30 rounded-2xl border border-[#1E293B]/50">
                    ...
                    {leg.commission > 0 && <span className="text-[11px] text-[#64748B]">Fee: ${leg.commission}</span>}
                  </div>
                ))}
            ...
            <CardBody className="p-8">
              ...
              <div className="grid grid-cols-1 gap-4">
                {trade.screenshots?.map((screenshot: any) => (
                  <div key={screenshot.id} className="relative aspect-video rounded-xl overflow-hidden group cursor-zoom-in border border-white/5">
                    <Image 
                      src={screenshot.url} 
